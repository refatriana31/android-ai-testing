---
id: 80-lib-m-018
title: Learner can narrow library content using tag or topic filters on mobile when tagging is enabled
priority: medium
timeout_seconds: 90
uat_id: LIB-M-018
...
- Precondition (from UAT): Library has content tagged by topic/category and filter UI is enabled
...
6. Open library; apply a tag or topic filter; review the result list

## Expected
- UAT expected: Only content matching the selected tag or topic appears and the selected filter state is visible
...
## Notes
- Auto-generated from UAT catalog row LIB-M-018. Refine wording as needed.
